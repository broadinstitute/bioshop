...
from .. models import ModelInputStruct, MultiModelInputStruct
...
from .. ann import AnnotationTransformer
...
def init_manager(batch_size=None, factor=4):
    ...
    manager.to_model = CircularBuffer(ctype=MultiModelInputStruct, size=maxsize)
...
def call_vcf(ref_path=None, vcf_in_path=None, vcf_out_path=None, model_path=None, batch_size=None, klen=None, region=None, n_workers=1):
    # XXX: annotations list
    num_transformer = AnnotationTransformer()
    num_transformer.train(vcf_path=vcf_in_path)
